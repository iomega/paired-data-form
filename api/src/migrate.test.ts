...
const EXAMPLE_PROJECT_JSON_V3: any = {
    'version': '3',
    'personal': {
        'submitter_name': 'Justin van der Hooft',
        'submitter_orcid': 'https://orcid.org/0000-0002-9340-5511',
        ...
        describe('with version 1 project', () => {
            it('should migrate', () => {
                ...
                console.log(
                    ...
                    EXAMPLE_PROJECT_JSON_V1.version
                    );
...
describe('migrateProject2to3 migration ', () => {
    let migration: Migration;
    ...
    describe('up()', () => {
        describe('with version 2 project', () => {
            ...
            beforeAll(() => {
                // migration happens in place, to compare later need a untouched example
                const copy = JSON.parse(JSON.stringify(EXAMPLE_PROJECT_JSON_V2));
                //
                copy.experimental.instrumentation_methods[0].column = 'HILIC';
                result = migration.up(copy);
            });

            it('should have version to 3', () => {
                expect(result.version).toEqual('3');
            });

            it('should have HILIC spelled out', () => {
                expect(result.experimental.instrumentation_methods[0].column).toEqual('Hydrophilic interaction (HILIC)');
            });

            it('should have empty proteomes', () => {
    ...
    describe('applicable()', () => {
        describe('with version 2 project', () => {
            it('should migrate', () => {
                ...
                console.log(
                    migration.applicable,
                    EXAMPLE_PROJECT_JSON_V2.version
                    );
                expect(result).toBeTruthy();
            });
        });
        ...
        describe('with version 3 project', () => {
            it('should not migrate', () => {
                const result = migration.applicable(EXAMPLE_PROJECT_JSON_V3);
                expect(result).toBeFalsy();
            });
        });
    });
...
describe('migrate()', () => {
    ...
    describe('with 3 approved and 3 pending project of version 1, 2 and 3', () => {
        beforeAll(() => {
            store = {
                listPendingProjects() {
                    return [{
                        ...
                        _id: 'somependingprojectid3',
                        project: JSON.parse(JSON.stringify(EXAMPLE_PROJECT_JSON_V3))
                    }];
                },
                listProjects() {
                    return [{
        ...
        it('should written migrated pending project', () => {
            expect(store.disk_store.writePendingProject).toHaveBeenCalledWith('somependingprojectid1', EXAMPLE_PROJECT_JSON_V3);
            expect(store.disk_store.writePendingProject).toHaveBeenCalledWith('somependingprojectid2', EXAMPLE_PROJECT_JSON_V3);
        });

        it('should written migrated prevously approved as pending project', () => {
            expect(store.disk_store.writePendingProject).toHaveBeenCalledWith('someapprovedprojectid1', EXAMPLE_PROJECT_JSON_V3);
            expect(store.disk_store.writePendingProject).toHaveBeenCalledWith('someapprovedprojectid2', EXAMPLE_PROJECT_JSON_V3);
        });
        ...
        it('should have approved the prevously approved project', () => {
            ...
            expect(store.disk_store.approveProject).toHaveBeenCalledWith('someapprovedprojectid2');
        });

        it('should not have written projects that where already of version 3', () => {
            expect(store.disk_store.writePendingProject).toHaveBeenCalledTimes(4);
            expect(store.disk_store.approveProject).toHaveBeenCalledTimes(2);
        });
    });
});
