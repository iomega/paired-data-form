...
export enum FilterFields {
    ...
    proteome_type = 'project.proteomes.proteome_ID.proteome_type_title.keyword',
...
export enum SortFields {
    ...
    nr_proteomes = 'summary.nr_proteomes',
...
export function expandEnrichedProjectDocument(project: EnrichedProjectDocument, schema: any) {
    ...
    if (!doc.project.proteomes) {
        doc.project.proteomes = [];
    }
    doc.project.proteomes.forEach((p: any) => {
        p.proteome_ID.proteome_type_title = 'Full proteome';
        if (p.proteome_ID.proteome_type === 'Enriched') {
            ...
            const targetsout = targets.map((t: any) => {
                if (t.target === 'other') {
                    ...
                    return lookups.proteome_target.get(t.target);
                }
            });
            p.proteome_ID.proteome_type_title = 'Enriched: ' + targetsout.join(', ');
        }
    });
